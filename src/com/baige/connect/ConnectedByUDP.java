...
//        }
    ...
    public SocketPacket sendPacket(SocketPacket packet) {
        if(packet == null){
            LogHelper.getInstance().error(this, "UDP发送失败：packet is null");
            return null;
        }
        if(getAddress() == null){
            LogHelper.getInstance().error(this, "UDP发送失败：address is null");
            return null;
        }
        if(getRunningSocket() == null){
            LogHelper.getInstance().error(this, "UDP发送失败：socket is null");
            return null;
        }
...
//                LogHelper.getInstance().verbose(this, "发送成功："+datagramPacket);
                return packet;
            }
        }else{
            LogHelper.getInstance().error(this, "UDP发送失败：address is invalid");
        }
        return null;
    }
    ...
    public String toString() {
        ...
        stringBuffer.append(", socket+"+getRunningSocket());
        stringBuffer.append(" }\n");
